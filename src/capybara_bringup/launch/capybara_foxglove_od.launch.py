#!/usr/bin/env python3
"""Foxglove bringup focused on object detection feeds only.

Launches the full Capybara stack (ZED, controllers) but the Foxglove bridge
only forwards ArUco and YOLO topics — no point cloud, depth, or nav topics.
Useful for inspecting detector output without saturating the bridge.
"""
...
def generate_launch_description():
    ...
    foxglove_bridge = Node(
        ...
        parameters=[
            PathJoinSubstitution([capybara_bringup_share, 'config', 'foxglove_bridge_od.yaml']),
